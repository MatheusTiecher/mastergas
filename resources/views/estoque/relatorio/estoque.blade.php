	...
	<table style="width: 100%;">
		<thead style="position:fixed">			
			<tr>
				...
				<th style="">LANÇAMENTO</th>
			</tr>
		</thead>
		@foreach($estoques as $estoque)
		<tr>
			<td style="">{{$estoque->id}}</td>
			<td style="">{{$estoque->produto->descricao}}</td>
			<td style="">{{$estoque->restante}} {{$estoque->produto->unidade->sigla}}</td>
			<td style="">{{date('d/m/Y H:i', strtotime($estoque->created_at))}}</td>
		</tr>
		@endforeach
	</table>
</div>
