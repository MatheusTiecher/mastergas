...
<div class="col-md-12">
	<div class="form-group">
		...
		<input type="text" name="anotacao" class="form-control form-control-md {{$errors->has('anotacao') ? 'is-invalid' : '' }}" placeholder="Anotação para entrega" id="anotacao" value="{{old('anotacao', isset($estoque->anotacao) ? $estoque->anotacao : null)}}">
...
<div class="col-md-12">
	<div class="form-group">
		<label for="ocorrencia" class="form-control-label">Motivo Ocorrência <abbr title="campo obrigatório">*</abbr></label>
		<input type="text" name="ocorrencia" class="form-control form-control-md {{$errors->has('ocorrencia') ? 'is-invalid' : '' }}" placeholder="Ocorrência entrega" id="ocorrencia" value="{{old('ocorrencia', isset($estoque->ocorrencia) ? $estoque->ocorrencia : null)}}">
		@if($errors->has('ocorrencia'))
		<div class="invalid-feedback">
			<p>{{ $errors->first('ocorrencia') }}</p>  
		</div>
		@endif
	</div>
</div>
...
<div class="col-md-12">
	<div class="form-group">
		...
		<div class="input-group input-group">
			...
			<input class="form-control form-control datepicker" placeholder="Select date" type="text" id="dataagendada" name="dataagendada" value="{{old('dataagendada', isset($venda->entrega->dataagendada) ? $venda->entrega->dataagendada : null)}}">
		</div>
	</div>
</div>
